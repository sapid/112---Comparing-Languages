...
if ($time == 0 and $has_tar){
    my @list_test = @{$target_hash{$myTarget}};
    $time = (stat($list_test[0]))[9];
}
if ($has_tar){
...
sub get_pre {
    ...
    foreach my $tar (@pre_list){
        ...
        if ($has_tar){
           ...
           &get_pre(\@pass_pre,$pre_time); # Recursively get the prereqs for this prereq.
        }
        if($pre_time and $pre_time > $timestamp){
           print "Newer!i $tar\n";
           print "Passed: $timestamp.  Compare: $pre_time\n";
           push(@pre_total, $tar);
        }
        elsif($pre_time){print "boobs\n"; }
        else{
           print "Doesnt exist! $tar\n";
           push(@pre_total, $tar);
        }
    }
}
